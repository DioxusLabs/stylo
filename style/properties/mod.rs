...
pub use style_traits::property_ids::{
    property_counts, AliasId, CountedUnknownProperty, IdSet, IdSetIterator, IndexedId,
    LogicalGroupId, LogicalGroupSet, LonghandId, LonghandIdSet, LonghandIdSetIterator,
    NonCustomPropertyId, NonCustomPropertyIdSet, NonCustomPropertyIdSetIterator,
    NonCustomPropertyIterator, PrioritaryPropertyId, PrioritaryPropertyIdSet,
    PrioritaryPropertyIdSetIterator, PropertyFlags, ShorthandId,
};

/// Gecko-binding conversions for property ids.
#[cfg(feature = "gecko")]
pub trait NonCustomPropertyIdGeckoExt: Sized {
    /// Convert a property id into a `NonCustomCSSPropertyId`.
    fn to_noncustomcsspropertyid(self) -> NonCustomCSSPropertyId;
    /// Convert a `NonCustomCSSPropertyId` into a property id.
    fn from_noncustomcsspropertyid(prop: NonCustomCSSPropertyId) -> Option<Self>;
}

#[cfg(feature = "gecko")]
impl NonCustomPropertyIdGeckoExt for NonCustomPropertyId {
    #[inline]
    fn to_noncustomcsspropertyid(self) -> NonCustomCSSPropertyId {
        // unsafe: guaranteed by static_assert_noncustomcsspropertyid.
        unsafe { mem::transmute(self.bit() as u16) }
    }
    ...
    fn from_noncustomcsspropertyid(prop: NonCustomCSSPropertyId) -> Option<Self> {
        ...
        Some(NonCustomPropertyId::from_index(prop))
    }
}

#[cfg(feature = "gecko")]
impl NonCustomPropertyIdGeckoExt for LonghandId {
    #[inline]
    fn to_noncustomcsspropertyid(self) -> NonCustomCSSPropertyId {
        NonCustomPropertyId::from(self).to_noncustomcsspropertyid()
    }

    fn from_noncustomcsspropertyid(id: NonCustomCSSPropertyId) -> Option<Self> {
...
#[cfg(feature = "gecko")]
impl NonCustomPropertyIdGeckoExt for ShorthandId {
    #[inline]
    fn to_noncustomcsspropertyid(self) -> NonCustomCSSPropertyId {
        NonCustomPropertyId::from(self).to_noncustomcsspropertyid()
    }
...
/// Methods on `ShorthandId` that depend on `PropertyDeclaration`.
pub trait ShorthandIdSerializationExt {
    /// Finds and returns an appendable value for the given declarations.
    ///
    /// Returns the optional appendable value.
    fn get_shorthand_appendable_value<'a, 'b: 'a>(
        self,
        declarations: &'a [&'b PropertyDeclaration],
    ) -> Option<AppendableValue<'a, 'b>>;
}

impl ShorthandIdSerializationExt for ShorthandId {
    fn get_shorthand_appendable_value<'a, 'b: 'a>(
        self,
        declarations: &'a [&'b PropertyDeclaration],
    ) -> Option<AppendableValue<'a, 'b>> {
...
impl From<NonCustomPropertyId> for PropertyId {
    #[inline]
    fn from(id: NonCustomPropertyId) -> Self {
